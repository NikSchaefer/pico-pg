...
import { testConnection, executeQuery, getTables, getTableColumns, updateLastConnectedTime } from '@/lib/db';
...
export function useQuery() {
  ...
  const runQuery = useCallback(async (connection: Connection, query: string) => {
    setIsLoading(true);
    setError(null);
...
export function useTableColumns(connection: Connection | null, table: string, schema: string = 'public') {
  const [isLoading, setIsLoading] = useState(false);
